# layout-txt-writer-skill — Snapshot 3

**Time:** 2026-08-09 00:11
**Kind:** message
...
decision (mine): include ALL THREE worked examples verbatim (login, card-grid, static-site shell) — thinker proposed 2; the LAYOUT.txt single-file form directly illustrates the Step-1 file-home decision, zero extra cost
decision (mine): skill-to-specs relative path pinned as ../../specs/LANGUAGE.md (from skills/layout-txt/)
decision (mine): check_layout.py tab rule interpreted — whitespace-only lines are blank (ignored); tab error applies to lines with content (G1 edge, documented in script header)
decision (mine): single commit docs(skills): add layout-txt writer skill (repo convention; feat() defensible since it ships a script — noted)
decision (mine): no .pi/settings.json created; symlink primary; fallback documented only
decision (mine): design-rationale pointer added to references/grammar.md (discussions/layout-spec-format/layout-txt-format.md) for traceability
...
- Grammar embedding: hybrid (b+) — compact quick-ref inline in SKILL.md; full restatement in references/grammar.md; explicit precedence "specs/LANGUAGE.md wins"; version pin LAYOUT v1 in 3 places
...
- Failure modes: 13-row markdown-habits-to-unlearn table (all verified against G1-G12)
...
- Q3 skill structure — answered: as above
...
### Next steps
- Present full proposal to user (question tool) — user gives opinion at this point only
- On approval: materialize files, validate, commit
